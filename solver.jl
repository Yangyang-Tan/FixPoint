...
function getlambda0(u1fun, u2fun, rho0::T, rhomin::T, d::T=T(3), n::T=T(1), η::T=T(0))
    ...
    function lossEigen(para)
        p = (d, n, η, para[2])
        f = (du, u, p, rho) -> Eigenfun(du, u, p, rho, U1, U2)
        u0 = iniEigen(para[1], rho0, d, η, para[2])
        prob = ODEProblem(f, u0, tspan, p)
        sol = solve(prob, Tsit5(); p=p, dense=false, save_on=false)
        loss = sum(abs2, sol .- 1)
        return loss, sol
    end
    return sol = solve(prob, method; abstol=atol, reltol=rtol, maxiters=10^7)
end
